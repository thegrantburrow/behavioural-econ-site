...
| 18 | Science Behind hero icon | `science-behind.html`, one per entry, top of the article; the same drawing repeats at compact scale in that entry's homepage teaser card (`.beats-feature .beats-icon`) | `.sb-hero-icon` (entry masthead), `.beats-feature .beats-icon` (homepage teaser, same paths, small scale) | 24x24 viewBox, stroke-width 1.7-1.8 (heavier than the 1.6 standard, since it reads as a mark, not a quiet tag). Must depict the literal real object the entry is about (a jean leg with a selvedge cuff, a portafilter, a ballot), never a generic stand-in category symbol, the same literal-object rule `principle-mechanism-diagram` already enforces for illustrations. Revised 2026-08-23: rendered at roughly 3.5-4x the standard eyebrow-icon scale (~110-120px icon in a ~150-160px tile) rather than the original ~2x, specifically so 2-4 small authentic surface details (a stitched pocket, rivets, a two-tone selvedge stripe, whatever the real object actually has) read clearly at that size instead of being lost. Uses the full site palette across those details, not one single accent: ink for the main outline, and terracotta, mustard, and teal each doing real work (an edge stripe, a stitch line, a rivet), not decoratively scattered. Governed by the `science-behind-article` skill. | Sits at the top of the entry, beside the title on wider screens (stacks above it on narrow ones), big enough to function as a masthead mark for that one piece, not a quiet inline tag like #1. The homepage teaser instance stays at the smaller `.beats-icon` scale used by every other teaser card. |
